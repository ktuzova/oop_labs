...
TEST(pentagon_test, default_constructor) {
    ASSERT_NO_THROW(Pentagon());
}

TEST(pentagon_test, point_constructor) {
    double R = 5;
    Point p1 = Point(R * std::cos(1 * 2 * M_PI / 5), R * std::sin(1 * 2 * M_PI / 5));
    Point p2 = Point(R * std::cos(2 * 2 * M_PI / 5), R * std::sin(2 * 2 * M_PI / 5));
    Point p3 = Point(R * std::cos(3 * 2 * M_PI / 5), R * std::sin(3 * 2 * M_PI / 5));
    Point p4 = Point(R * std::cos(4 * 2 * M_PI / 5), R * std::sin(4 * 2 * M_PI / 5));
    Point p5 = Point(R * std::cos(5 * 2 * M_PI / 5), R * std::sin(5 * 2 * M_PI / 5));
    ASSERT_NO_THROW(Pentagon(p1, p2, p3, p4, p5));
    EXPECT_THROW(Pentagon(Point(-100.0, -100.0), p2, p3, p4, p5), std::logic_error);
}
TEST(pentagon_test, side_constructor) {
    ASSERT_NO_THROW(Pentagon(4));
    EXPECT_THROW(Pentagon(-4), std::logic_error);
}
TEST(pentagon_test, equality) {
    ...
    Point p4 = Point(R * std::cos(4 * 2 * M_PI / 5), R * std::sin(4 * 2 * M_PI / 5));
    Point p5 = Point(R * std::cos(5 * 2 * M_PI / 5), R * std::sin(5 * 2 * M_PI / 5));
    Pentagon h0(p1, p2, p3, p4, p5);
    Pentagon h1(p1, p2, p3, p4, p5);
    Pentagon h2(h1);
    Pentagon h3(146);
    ASSERT_TRUE(h1 == h0);
    ASSERT_TRUE(h2 == h0);
...
TEST(pentagon_test, area_test) {
    Pentagon h0(3);
    ASSERT_TRUE(fabs(h0.area() - (0.25 * sqrt (5 + 2 * sqrt(5)) * 3 * 3)) <= FLT_EPSILON);
}
TEST(pentagon_test, double_test) {
    ...
    Point p5 = Point(R * std::cos(5 * 2 * M_PI / 5) , R * std::sin(5 * 2 * M_PI / 5) );
    Pentagon h0(p1, p2, p3, p4, p5);
    ASSERT_TRUE(fabs(static_cast<double>(h0) - (0.25 * sqrt (5 + 2 * sqrt(5)) * 3 * 3)) <= FLT_EPSILON);
}
TEST(pentagon_test, geom_center) {
    Pentagon h0(3);
    double R = 6;
    Point p1 = Point(R * std::cos(1 * 2 * M_PI / 5) + 2, R * std::sin(1 * 2 * M_PI / 5) + 2);
    Point p2 = Point(R * std::cos(2 * 2 * M_PI / 5) + 2, R * std::sin(2 * 2 * M_PI / 5) + 2);
    Point p3 = Point(R * std::cos(3 * 2 * M_PI / 5) + 2, R * std::sin(3 * 2 * M_PI / 5) + 2);
    Point p4 = Point(R * std::cos(4 * 2 * M_PI / 5) + 2, R * std::sin(4 * 2 * M_PI / 5) + 2);
    Point p5 = Point(R * std::cos(5 * 2 * M_PI / 5) + 2, R * std::sin(5 * 2 * M_PI / 5) + 2);
    Pentagon h1(p1, p2, p3, p4, p5);
